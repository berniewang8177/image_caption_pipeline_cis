...
import torch
from model_attention import SCAN, xattn_score_t2i, xattn_score_i2t
...
class LogCollector(object):
    ...
    def __str__(self):
        ...
        """
        s = ''
        for i, (k, v) in enumerate(self.meters.items()):  # iteritems change to items
            if i > 0:
                s += '  '
            s += k + ' ' + str(v)
        return s
...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    for i, (images, boxes, captions, lengths, ids) in enumerate(data_loader):
        ...
        img_emb, cap_emb, cap_len = model.forward_emb(images, boxes, captions, lengths, volatile=True)
        #print(img_emb)
        if img_embs is None:
            if img_emb.dim() == 3:
...
def shard_xattn_t2i(images, captions, caplens, opt, shard_size=128):
    ...
    n_im_shard = int(n_im_shard)                  # float?
    n_cap_shard = int(n_cap_shard)               # float?
    
    for i in range(n_im_shard): # float object????
