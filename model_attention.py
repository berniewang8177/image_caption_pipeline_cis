...
class EncoderImagePrecomp(nn.Module):
    ...
    def __init__(self, img_dim, embed_size, no_imgnorm=False, split_size = 16, position_embed_size = 200):
    ...
    def attention_forward(self, images, box_features):
        ...
        cos_sim = F.softmax(cos_sim, -1) # nn.Softmax()(cos_sim)
...
#         new_boxes_index = new_boxes[:,:new_boxes.size(1)/2].type(torch.LongTensor)
        new_boxes_index = new_boxes[:,:new_boxes.size(1)//2].type(torch.LongTensor)
#         new_boxes_weight = new_boxes[:,new_boxes.size(1)/2:]
        new_boxes_weight = new_boxes[:,new_boxes.size(1)//2:]
        ...
        new_boxes_weight = F.softmax(new_boxes_weight, -1) # nn.Softmax()(new_boxes_weight)
...
def xattn_score_t2i(images, captions, cap_lens, opt):
    ...
    for i in range(n_caption):
        ...
        #print "Attn", attn
        #print "Max attn2", max_attn2
        #print "Diff", diff_attn
        ...
        similarities.append(row_sim+0.5*max_attn) # +0.5*diff_score
...
class SCAN(object):
    ...
    def forward_emb(self, images, boxes, captions, lengths, volatile=False):
        ...
        with torch.no_grad():        # because  volatile was removed and now has no effect
            if torch.cuda.is_available():
                images = images.cuda()
                boxes = boxes.cuda()
        ...
        # Forward
        img_emb = self.img_enc(images, boxes)
    ...
    def train_emb(self, images, boxes, captions, lengths, ids=None, *args):
        ...
        img_emb, cap_emb, cap_lens = self.forward_emb(images, boxes, captions, lengths)
